...
INSERT INTO "golden_regression" ("id", "key", "description", "expectedResult", "testReference", "status", "lastCheckedAt", "updatedAt") VALUES
('gr_conduit_offset_6in_30deg', 'conduit-offset-6in-30deg', '6in offset at a 30-degree bend must yield 12in spacing (spacing = offset / sin(angle)).', 'spacingIn = 12', 'packages/tools/test/conduit-offset.test.ts', 'PASSING', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('gr_payment_release_canonical', 'payment-release-canonical-path', 'All payment release paths (UI/agent/workflow) must converge on the single escrow-release command.', 'One canonical command path, no direct writes elsewhere', 'not yet wired to an automated regression', 'NOT_WIRED', NULL, CURRENT_TIMESTAMP),
('gr_privacy_local_fallback', 'privacy-local-only-fallback', 'Cross-tenant / cross-actor requests must never leak another tenant''s data via a local/offline fallback path.', 'No cross-tenant data in response', 'not yet wired to an automated regression', 'NOT_WIRED', NULL, CURRENT_TIMESTAMP),
('gr_migration_startup', 'migration-startup-safety', 'A failed migration must block subsequent deploys with a clear, resolvable error rather than silently skipping or corrupting state.', 'migrate deploy fails loudly, resolve path documented', 'not yet wired to an automated regression', 'NOT_WIRED', NULL, CURRENT_TIMESTAMP),
('gr_cross_tenant_isolation', 'cross-tenant-isolation', 'No resource read/write may cross tenant boundaries regardless of actor role.', 'Cross-tenant access denied', 'not yet wired to an automated regression', 'NOT_WIRED', NULL, CURRENT_TIMESTAMP),
('gr_duplicate_command', 'duplicate-command-rejection', 'The same idempotency key must not produce two distinct side effects.', 'Second call is a no-op returning the first result', 'not yet wired to an automated regression', 'NOT_WIRED', NULL, CURRENT_TIMESTAMP),
('gr_duplicate_event', 'duplicate-event-dedupe', 'Re-delivered domain events must be deduped by consumers.', 'Second delivery has no additional side effect', 'not yet wired to an automated regression', 'NOT_WIRED', NULL, CURRENT_TIMESTAMP),
('gr_stale_approval', 'stale-approval-rejection', 'An approval based on a stale expectedVersion must be rejected, not silently applied.', 'Stale approval returns a version-conflict error', 'not yet wired to an automated regression', 'NOT_WIRED', NULL, CURRENT_TIMESTAMP),
('gr_offline_conflict', 'offline-conflict-resolution', 'Offline-queued writes that conflict with server state on reconnect must be surfaced, not silently overwritten.', 'Conflict surfaced to the actor, not auto-resolved', 'not yet wired to an automated regression', 'NOT_WIRED', NULL, CURRENT_TIMESTAMP);
