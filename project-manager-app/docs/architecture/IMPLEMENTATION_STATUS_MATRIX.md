...
| Tool Registry | PARCIAL | 31 descriptors (24 read + 7 write). Gobernanza F2 completa y en `main` (PRs #369/#371/#372): `evaluatePrometeoToolPolicy` gatea el 100% de invocaciones (no solo `agents:run:create`), `PrometeoToolInvocationAudit` registra cada intento, `PrometeoProposedAction` exige aprobacion humana antes de ejecutar escritura de riesgo medio/alto/critico. 18/24 read cableados, 7/7 write cableados (incluye `payments.propose_release` -> `PaymentsService.release()` real via aprobacion) | 6 tools `vision.*` quedan `adapter_pending`: `analyze_image`, `compare_before_after`, `detect_material`, `classify_space`, `check_safety`, `analyze_video` (este ultimo ademas requiere pipeline temporal) |
