...
- **Date:** 2026-09-18
- **Status:** Proposed — human GO received 2026-09-18 for "first compatibility slice only", then **partially superseded same day**: the context-switch mechanism (`OrgContextService`, `/v1/me/*` endpoints) is `BLOCKED` pending reconciliation with `docs/specs/core/universal-identity-multi-role.spec.md` (see Addendum below); only the `Membership.status` lifecycle piece proceeded to implementation.
