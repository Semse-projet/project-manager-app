...
3. **Rango completo de valores de `Org.type` en producción real (no solo el seed).** Bloqueo: no se consultó la base de datos de producción en esta pasada, solo el seed de desarrollo. Próxima acción: query de solo lectura contra producción (`SELECT DISTINCT type FROM "Org"`) antes de decidir si el eje actual ya equivale a `PERSONAL/BUSINESS/PLATFORM` o necesita extenderse. **Sigue abierto** — requiere abrir un proxy TCP temporal a Postgres de producción, acción que se le debe pedir explícitamente al usuario antes de ejecutar.
4. **Cuántos usuarios reales tienen hoy más de un `Membership`.** Bloqueo: mismo motivo — requiere datos de producción, no solo el modelo. Próxima acción: query de solo lectura antes de dimensionar el impacto real del primer slice de context switching. **Sigue abierto**, mismo motivo que el punto 3.
...
**Nota sobre el estado de este gate (owner, 2026-09-18, actualizado tras el cierre de los ítems 1, 2 y 5):** de los 5 `BLOCKED` originales, 3 quedaron resueltos con evidencia real (PRs #656/#658, y la auditoría de `landing-intake.tsx`); los ítems 3 y 4 siguen abiertos porque requieren acceso a producción, no solo a código. El estado del gate sigue siendo **`PASS WITH BLOCKERS FOR DOWNSTREAM`** hasta que 3 y 4 se resuelvan — ya no por los riesgos P0 de payment-governance/BuildOps (cerrados), sino por la falta de datos reales de producción para dimensionar el rollout del primer slice de context switching. El hallazgo nuevo de `public/professionals/preview` (ítem 5) es una pregunta de producto abierta, no un blocker de seguridad confirmado.
