...
> **Nota de alineación:** `READY` es solo una etiqueta de producto para el valor
> persistido `AWAITING_REVIEW`. El submit también acepta `DRAFT` y `REJECTED`,
> siempre con evidencia, y persiste directamente `SUBMITTED`.
...
|-------|-----------|-------------|----------------|
| Profesional | `PRO` | Someter a revisión milestones del proyecto asignado | Crear en proyectos ajenos, aprobar o rechazar |
| Cliente | `CLIENT` | Crear, aprobar, rechazar o pedir cambios en su proyecto | Submit como profesional |
...
|-------|-------|-------|-------|--------|
| `DRAFT` / `AWAITING_REVIEW` / `REJECTED` | `SUBMITTED` | PRO asignado u OPS · evidencia mínima adjunta | PRO / OPS_ADMIN | `milestone.submitted` event · SSE · notifica CLIENT |
...
| `SUBMITTED` / `APPROVED` | `REJECTED` | Razón explícita; `PAID` sigue protegido | CLIENT / OPS_ADMIN | `milestone.rejected` event · notifica PRO |
| `SUBMITTED` / `APPROVED` | `DRAFT` | Razón explícita mediante `request-changes` | CLIENT / OPS_ADMIN | `milestone.revision_requested` event |
...
- Sin evidencia no puede transicionarse de `DRAFT`, `AWAITING_REVIEW` o `REJECTED` a `SUBMITTED`
...
```
DADO   que existe un milestone en DRAFT, AWAITING_REVIEW o REJECTED con evidencia adjunta
...
**Casos borde:**
- Milestone sin evidencia → `409 Conflict`
...
**Casos borde:**
- Milestone en estado distinto a `SUBMITTED` → `409 Conflict`
...
- Milestone en estado distinto a `SUBMITTED` o `APPROVED` → `409 Conflict`; `PAID` es terminal

---

### P2-A — Cliente crea milestone en proyecto

**Journey:** Al inicio del proyecto, el cliente dueño (o OPS) define los milestones con título, monto y secuencia.
...
errores:
  409: milestone no está en DRAFT/awaiting_review/rejected o no tiene evidencia
...
efectos:
  ...
  fsmTransicion: DRAFT|AWAITING_REVIEW|REJECTED → SUBMITTED
  paymentGovernance: false
...
errores:
  409: milestone no está en estado SUBMITTED o APPROVED; PAID nunca se rechaza
...
errores:
  ...
  409: milestone no está en estado SUBMITTED o APPROVED; PAID es terminal
...
efectos:
  ...
  fsmTransicion: SUBMITTED|APPROVED → REJECTED
  paymentGovernance: false
```
...
método: POST
roles: [CLIENT, OPS_ADMIN]
permiso: milestones:reject
input: { reason: string trim().min(1) }
output: MilestoneRecord en DRAFT con reviewDecision=request_changes
errores:
  400: reason vacío
  409: estado distinto de SUBMITTED/APPROVED o estado PAID
efectos:
  auditLog: milestone.request_changes
  evento: milestone.revision_requested
  fsmTransicion: SUBMITTED|APPROVED → DRAFT
```
...
describe("POST /v1/milestones/:id/submit") {
  it("PRO puede someter DRAFT, AWAITING_REVIEW o REJECTED con evidencia")
  it("rechaza si no hay evidencia")
...
describe("POST /v1/milestones/:id/reject") {
  ...
  it("permite corregir APPROVED antes del pago y rechaza PAID")
...
| Gap | Tipo | Severidad |
|-----|------|-----------|
| Comentarios opcionales de aprobación todavía no se persisten | Contrato | 🟢 Baja |
...
| `request_changes` vuelve a `DRAFT` y queda tipado en `reviewDecision`; no crea un estado persistido adicional | FSM | ✅ Cerrado |
