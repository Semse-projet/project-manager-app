...
### G-PRO-02 — PARCIALMENTE RESUELTO — Cuenta profesional sin Stripe Connect conectado
...
**UI (el "Fix esperado" original de este gap) = plan 2.1c, SIGUE ABIERTO:** `[ ] Pendiente — ver 0.16`, sin cambios desde el registro original. `/worker/payments` sigue mostrando el CTA neutral de "crea una cuenta" en vez de explicar que el payout queda bloqueado sin ella. No implementado a ciegas — queda como gap real de UI para una próxima pasada.
...
### G-PRO-04 — SIGUE ABIERTO — Verificación de identidad (firma DID) es un stub, expuesto como "Sin verificar" al profesional
**= plan 0.9 + nota en 3.13.** **No corregido, deliberadamente.** El propio plan lo dice explícito: hacer que la verificación DID funcione de punta a punta es una decisión de producto (¿quién aporta la firma criptográfica real — el propio worker, o un proceso separado?), no un fix de una línea — se investigó hasta la causa raíz y se dejó señalado, sin implementar a ciegas. Sigue contradiciendo la promesa de marca "Profesionales verificados" de la landing pública. Este es el único gap `CRÍTICO` de este spec que sigue genuinamente sin resolver.
...
- [ ] `/worker/payments` comunica explícitamente por qué no se puede cobrar cuando no hay cuenta Connect activa — **sigue sin hacerse** (plan 2.1c, `[ ] Pendiente`), ver G-PRO-02
...
- [ ] Un usuario con rol `WORKER` (literal, no alias de PRO) tiene el mismo resultado — el fix cubre a `WORKER` en `rbac.ts` igual que a `PRO`, pero no se repitió la verificación en vivo con una cuenta `WORKER` literal por separado
...
- [x] Antes de `APPROVED`: el equipo de producto revisa y prioriza los 42+13 hallazgos de este spec — hecho de facto entre 2026-07-21 y 2026-07-27: 11 de los 13 gaps `G-PRO-*` propios ya tienen fix (algunos con decisión de producto explícita del usuario), más las piezas de seguridad del plan (2.19/2.20/2.34/2.44). Quedan genuinamente abiertos: **G-PRO-04** (verificación DID, bloqueado en una decisión de producto sobre quién aporta la firma real) y la mitad UI de **G-PRO-02** (mensaje claro de "no podés cobrar sin Connect", plan 2.1c). Los ~55 hallazgos `ALTO`/`MEDIO`/`BAJO` de 2.8-2.48 no cubiertos como gap propio siguen sin revisión de priorización dedicada — no se afirma que estén todos resueltos.
