...
- [x] Antes de `APPROVED`: el equipo de producto revisa y prioriza los 42+13 hallazgos de este spec — hecho de facto entre 2026-07-21 y 2026-07-27: 12 de los 13 gaps `G-PRO-*` propios ya tienen fix (algunos con decisión de producto explícita del usuario, la mitad UI de G-PRO-02 cerrada 2026-08-27), más las piezas de seguridad del plan (2.19/2.20/2.34/2.44). Queda genuinamente abierto: **G-PRO-04** (verificación DID, bloqueado en una decisión de producto sobre quién aporta la firma real). Los ~55 hallazgos `ALTO`/`MEDIO`/`BAJO` de 2.8-2.48 no cubiertos como gap propio siguen sin revisión de priorización dedicada — no se afirma que estén todos resueltos.
