...
- Railway/API deployed SHA: **RESOLVED, D02 mitigation + D08 all closed for API.** `railway redeploy --from-source` of `71467441` (PR #622/#623 merge) confirmed live: `GET https://api.semseproject.com/v1/health` returns `gitSha: "71467441d3e7f05458cb18565de1e7db62036a18"`. See "Last verified production behavior" below.
- Railway/Web deployed SHA: **RESOLVED.** ADR-033 verified live: `GET https://app.semseproject.com/api/semse/healthz` returns `gitSha: "71467441d3e7f05458cb18565de1e7db62036a18"`.
- Railway/Worker deployed SHA: **RESOLVED.** ADR-033 verified live via `railway logs --service semse-worker`: startup diagnostic line shows `gitSha="71467441d3e7f05458cb18565de1e7db62036a18"`.
- Production verification performed today: all three services redeployed together (`railway redeploy --from-source`) from commit `71467441` and confirmed running it (see "Last verified production behavior").
...
| ~~D08 — Web/Worker provenance closed in code (ADR-033), not yet production-verified~~ — **RESOLVED 2026-09-14.** `railway redeploy --from-source` for semse-API/web/worker from commit `71467441`; `GET /api/semse/healthz` and `railway logs --service semse-worker` both confirmed the real `gitSha`. See "Last verified production behavior". | — | — | — |
...
2026-09-14T12:4x UTC — following the PR #622/#623 merges (squash commit `71467441d3e7f05458cb18565de1e7db62036a18`), ran `railway redeploy --from-source` for all three application services together (semse-API, semse-web, semse-worker) and confirmed each is running that exact commit:
- `GET https://api.semseproject.com/v1/health` → `{"status":"ok","service":"semse-api","persistence":"prisma","gitSha":"71467441d3e7f05458cb18565de1e7db62036a18","buildTime":"unknown",...}`.
- `GET https://app.semseproject.com/api/semse/healthz` → `{"data":{"status":"ok","service":"semse-web","gitSha":"71467441d3e7f05458cb18565de1e7db62036a18","buildTime":"unknown"}}` — closes ADR-033/D08 for Web.
- `railway logs --service semse-worker` startup line → `gitSha="71467441d3e7f05458cb18565de1e7db62036a18"` — closes ADR-033/D08 for Worker.
- `POST https://api.semseproject.com/v1/payments/release` (no auth) → `401` (route mounted, guard active, unchanged from before the fix) — confirms the deployed code includes the ADR-034 controller; the disabled-path `503` body itself was not re-verified against prod with real admin credentials (covered by the passing local test suite instead).
- `buildTime` is `"unknown"` for all three — `RAILWAY_DEPLOYMENT_CREATED_AT` isn't populated for this deploy method; expected fallback behavior per ADR-030/033, not a bug.

This closes D08 fully (API+Web+Worker, all production-verified) and closes the immediate D02 money-safety incident (mitigated and live) — the D02 *design* work (real delegation target, milestone resolution, duplicate-class reconciliation) remains open, see Blockers.
