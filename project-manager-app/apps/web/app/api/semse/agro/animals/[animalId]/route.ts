...
export async function GET(req: NextRequest, { params }: { params: Promise<{ animalId: string }> }) {
  ...
  try {
    ...
    const data = await fetchSemseDataForRequest<unknown>(`/v1/agro/animals/${animalId}`, req);
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ animalId: string }> }) {
  if (!isSemseRuntimeEnabled()) return runtimeDisabledResponse();
  try {
    const { animalId } = await params;
