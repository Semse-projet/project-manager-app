...
export async function GET(req: NextRequest, { params }: { params: Promise<{ farmId: string }> }) {
  ...
  try {
    ...
    const data = await fetchSemseDataForRequest<unknown>(`/v1/agro/farms/${farmId}/production${qs ? `?${qs}` : ""}`, req);
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ farmId: string }> }) {
  ...
  try {
    ...
    const data = await fetchSemseDataForRequest<unknown>(`/v1/agro/farms/${farmId}/production`, req, {
      method: "POST",
