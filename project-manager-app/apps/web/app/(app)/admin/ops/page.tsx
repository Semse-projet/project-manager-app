...
export default function AdminOpsPage() {
  ...
  return (
    <div style={{ maxWidth: "1260px", margin: "0 auto", display: "grid", gap: "18px" }}>
      <section
        style={{
      ...
      >
        <article style={{ border: "1px solid var(--border)", borderRadius: "16px", background: "var(--surface)", padding: "18px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: "12px", flexWrap: "wrap", marginBottom: "14px" }}>
            <div>
              <h2 style={{ fontSize: "18px", fontWeight: 800, color: "var(--ink)" }}>Filtros activos</h2>
              <p style={{ fontSize: "12px", color: "var(--muted)" }}>Controla el runtime por evento, estado y agente antes de abrir el trace.</p>
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "minmax(220px, 1.1fr) repeat(3, minmax(140px, 0.7fr))", gap: "10px" }}>
            <label style={{ position: "relative" }}>
              <Search size={14} style={{ position: "absolute", left: "12px", top: "50%", transform: "translateY(-50%)", color: "var(--muted)" }} />
              <input
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Buscar correlationId, agente o evento"
                style={{
                  width: "100%",
                  height: "40px",
                  paddingLeft: "36px",
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(140px, 1fr))", gap: "10px", marginTop: "10px" }}>
            {[
        ...
        <article style={{ border: "1px solid var(--border)", borderRadius: "16px", background: "var(--surface)", padding: "18px" }}>
          <h2 style={{ fontSize: "18px", fontWeight: 800, color: "var(--ink)", marginBottom: "10px" }}>Lectura operativa</h2>
      ...
      >
        <article style={{ border: "1px solid var(--border)", borderRadius: "16px", background: "var(--surface)", padding: "18px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: "12px", marginBottom: "12px", alignItems: "center" }}>
            <div>
              <h2 style={{ fontSize: "18px", fontWeight: 800, color: "var(--ink)" }}>Domain events</h2>
              <p style={{ fontSize: "12px", color: "var(--muted)" }}>Plano causal: eventos emitidos antes del runtime de agentes.</p>
            </div>
        ...
        <article style={{ border: "1px solid var(--border)", borderRadius: "16px", background: "var(--surface)", padding: "18px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: "12px", marginBottom: "12px", alignItems: "center" }}>
            <div>
              <h2 style={{ fontSize: "18px", fontWeight: 800, color: "var(--ink)" }}>Event trace</h2>
              <p style={{ fontSize: "12px", color: "var(--muted)" }}>Meta, payload y timeline de auditoría del evento raíz.</p>
            </div>
            {domainEventTraceLoading ? <span style={{ fontSize: "12px", color: "var(--muted)" }}>Cargando trace...</span> : null}
          </div>

          {domainEventTraceError ? <p style={{ fontSize: "12px", color: "#fca5a5", marginBottom: "10px" }}>{domainEventTraceError}</p> : null}
          ...
          {selectedEvent ? (
            <div style={{ display: "grid", gap: "12px" }}>
              <div style={{ borderRadius: "14px", border: "1px solid rgba(196,181,253,0.2)", background: "rgba(76,29,149,0.12)", padding: "14px" }}>
                ...
                <p style={{ fontSize: "12px", color: "var(--muted)", marginTop: "6px", wordBreak: "break-word" }}>{selectedEvent.correlationId}</p>
            ...
            <div style={{ borderRadius: "14px", border: "1px dashed rgba(148,163,184,0.24)", padding: "16px", color: "var(--muted)" }}>
              Selecciona un `correlationId` para inspeccionar el evento raíz.
            </div>
          )}
        </article>
      </section>
      ...
      >
        <article style={{ border: "1px solid var(--border)", borderRadius: "16px", background: "var(--surface)", padding: "18px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: "12px", marginBottom: "12px", alignItems: "center" }}>
            <div>
              <h2 style={{ fontSize: "18px", fontWeight: 800, color: "var(--ink)" }}>Corridas recientes</h2>
              <p style={{ fontSize: "12px", color: "var(--muted)" }}>Lista filtrada de runs recientes cargados desde Ops API.</p>
            </div>
            ...
            {visibleItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", gap: "10px", marginTop: "10px", fontSize: "11px", color: "var(--muted)", flexWrap: "wrap" }}>
                    <span>Intentos {item.attempts}/{item.maxAttempts}</span>
                    <span>{item.requiresHumanReview ? "Human review" : "Auto-flow"}</span>
                    <span>{formatTimestamp(item.updatedAt)}</span>
                  </div>
                  {item.outputSummary ? (
                    <p style={{ fontSize: "11px", color: "#a5f3fc", marginTop: "6px", fontStyle: "italic", textAlign: "left" }}>
                      {item.outputSummary}
                    </p>
                  ) : null}
                  {(() => {
                    ...
                    return next ? (
                      <p style={{ fontSize: "11px", color: "#fbbf24", marginTop: "4px", fontWeight: 600, textAlign: "left" }}>
                        ▶ {next}
                      </p>
                    ) : null;
                  })()}
        ...
        <article style={{ border: "1px solid var(--border)", borderRadius: "16px", background: "var(--surface)", padding: "18px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: "12px", marginBottom: "12px", alignItems: "center", flexWrap: "wrap" }}>
            <div>
              <h2 style={{ fontSize: "18px", fontWeight: 800, color: "var(--ink)" }}>Trace auditable</h2>
              <p style={{ fontSize: "12px", color: "var(--muted)" }}>Evento raíz, runs disparados y línea temporal completa.</p>
            </div>
          ...
          {runtimeTrace ? (
            <div style={{ display: "grid", gap: "12px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: "10px" }}>
                {[
                  { label: "Triggers", value: runtimeTrace.summary.triggerCount },
                  { label: "Queued", value: runtimeTrace.summary.queued },
                  { label: "Completed", value: runtimeTrace.summary.completed },
                  { label: "Failed", value: runtimeTrace.summary.failed }
                ].map((card) => (
                  <div key={card.label} style={{ borderRadius: "12px", border: "1px solid rgba(148,163,184,0.16)", background: "rgba(15,23,42,0.44)", padding: "12px" }}>
                    <span style={{ display: "block", fontSize: "11px", color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.06em" }}>{card.label}</span>
                    <strong style={{ display: "block", marginTop: "6px", fontSize: "24px", color: "var(--ink)" }}>{card.value}</strong>
                  </div>
                ))}
              </div>

              {traceHasFailures ? (
                <div style={{ borderRadius: "14px", border: "1px solid rgba(239,68,68,0.22)", background: "rgba(239,68,68,0.07)", padding: "14px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
                    <p style={{ fontSize: "12px", color: "#fca5a5" }}>
                      Hay runs con fallo en este trace.
                    </p>
                    {!showIncidentForm ? (
                      <button
                        type="button"
                  ...
                  {incidentSuccess ? (
                    <p style={{ fontSize: "12px", color: "#34d399", marginTop: "8px" }}>{incidentSuccess}</p>
                  ) : null}
                  {showIncidentForm ? (
                    <form
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
                        ...
                        >
                          {incidentLoading ? "Enviando..." : "Confirmar"}
                        </button>
                        <button
                          type="button"
                      ...
                      {incidentError ? <p style={{ fontSize: "11px", color: "#fca5a5" }}>{incidentError}</p> : null}
                    </form>
                  ) : null}
                </div>
              ) : null}
                  ...
                  {traceRuns.map((run) => {
                    ...
                    return (
                      <div key={run.id} style={{ borderRadius: "12px", border: "1px solid rgba(148,163,184,0.12)", padding: "10px 12px" }}>
                        ...
                        <p style={{ fontSize: "12px", color: "var(--muted)", marginTop: "4px" }}>{run.triggerType} · {run.workerId ?? "worker-pending"}</p>
                        {run.outputSummary ? (
                          <p style={{ fontSize: "12px", color: "#a5f3fc", marginTop: "6px", fontStyle: "italic" }}>
                            {run.outputSummary}
                          </p>
                        ) : null}
                        {(() => {
                          ...
                          return next ? (
                            <p style={{ fontSize: "11px", color: "#fbbf24", marginTop: "4px", fontWeight: 600 }}>
                              ▶ {next}
                            </p>
                          ) : null;
                        })()}
                        {actErr ? <p style={{ fontSize: "11px", color: "#fca5a5", marginTop: "4px" }}>{actErr}</p> : null}
                        {run.operatorContext ? (
              ...
              {runtimeTrace.workspaceMemory.length > 0 ? (
                <div style={{ borderRadius: "14px", border: "1px solid rgba(34,211,238,0.18)", background: "rgba(8,47,73,0.18)", padding: "14px" }}>
                  ...
                  <div style={{ display: "grid", gap: "8px" }}>
                    {runtimeTrace.workspaceMemory.map((memory) => (
                      <div key={memory.id} style={{ borderRadius: "12px", border: "1px solid rgba(125,211,252,0.12)", padding: "10px 12px" }}>
                        <strong style={{ display: "block", fontSize: "13px", color: "var(--ink)" }}>{memory.title}</strong>
                        <p style={{ fontSize: "12px", color: "var(--muted)", marginTop: "4px" }}>{memory.summary}</p>
                        <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginTop: "8px" }}>
            ...
            <div style={{ borderRadius: "14px", border: "1px dashed rgba(148,163,184,0.24)", padding: "16px", color: "var(--muted)" }}>
              Selecciona una corrida para ver el trace completo.
            </div>
          )}
        </article>
      </section>
