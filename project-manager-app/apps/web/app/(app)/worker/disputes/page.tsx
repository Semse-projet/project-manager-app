...
const STATUS_META: Record<DisputeRow["status"], { variant: "error" | "warning" | "success"; label: string; tone: string }> = {
  ...
  under_review: { variant: "warning", label: "En revisión", tone: "var(--warn)" },
...
export default function WorkerDisputesPage() {
  ...
  const stablePathname = pathname ?? "/worker/disputes";
  ...
  return (
    <div style={{ maxWidth: "1180px", margin: "0 auto", display: "grid", gap: 18 }}>
      <HtmlInCanvasPanel as="section" style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }} canvasClassName="rounded-2xl" minHeight={96}>
        <div>
          ...
          <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: "var(--ink)" }}>Disputas</h1>
          <p style={{ margin: "6px 0 0", fontSize: 13, color: "var(--muted)", maxWidth: 760 }}>
            Gestiona bloqueos con herramientas de autogestión: evidencia, criterio del copiloto y escalación asistida cuando ya no se puede resolver entre las partes.
          </p>
        </div>
        ...
        <div style={{ display: "grid", gap: 12 }}>
          <div>
            ...
            <p style={{ margin: "4px 0 0", fontSize: 12, color: "var(--muted)" }}>
              Cuando el desacuerdo no se pueda resolver con mensajes normales, abre una disputa formal y documenta el bloqueo.
            </p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "minmax(220px, 280px) 1fr auto", gap: 10, alignItems: "start" }}>
            ...
            <textarea
              ...
              placeholder="Explica qué entregable, pago o evidencia está trabado."
              rows={2}
      ...
      <HtmlInCanvasPanel as="section" canvasClassName="rounded-2xl" minHeight={68}>
      ...
      {loading ? (
        ...
        <HtmlInCanvasPanel as="section" style={{ padding: "42px 24px", textAlign: "center" }} canvasClassName="rounded-2xl" minHeight={180}>
          ...
          <p style={{ fontSize: 13, color: "var(--muted)", marginTop: 4 }}>
            Si surge un bloqueo operativo o económico, ábrelo y documenta la situación antes de escalar.
          </p>
        </HtmlInCanvasPanel>
      ) : (
          ...
          {selectedDispute ? (
            <div style={{ position: "sticky", top: 16, display: "grid", gap: 10 }}>
              ...
              <DisputeResolutionWorkspace
                ...
                audience="worker"
                canResolve={false}
                relatedHref={selectedDispute.jobId ? `/worker/jobs/${selectedDispute.jobId}` : undefined}
                evidenceHref={selectedDispute.jobId ? `/worker/evidence?jobId=${encodeURIComponent(selectedDispute.jobId)}` : "/worker/evidence"}
                documentsHref={selectedDispute.jobId ? `/worker/jobs/${selectedDispute.jobId}#milestones-section` : undefined}
              />
            </div>
          ) : null}
        </div>
