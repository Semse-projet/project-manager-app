...
export class AgroAnimalService {
  ...
  async updateAnimal(animalId: string, ownerId: string, input: {
    tagCode?: string;
    breed?: string;
    birthDate?: Date;
    estimatedAgeMonths?: number;
  ...
  async changeAnimalStatus(animalId: string, ownerId: string, status: string, reason?: string) {
  ...
  async changeGroupStatus(groupId: string, ownerId: string, status: string, reason?: string) {
    if (!VALID_STATUS.includes(status as any)) {
      throw new BadRequestException(`Invalid status: ${status}`);
    }
    const group = await this.getGroup(groupId);
