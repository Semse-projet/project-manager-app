...
import { OewsService, STATIC_LABOR_MULTIPLIERS } from "./oews.service.js";
import type { LocationMultipliers } from "@semse/tools";

const CACHE_TTL_MS = 7 * 24 * 60 * 60 * 1000; // 7 days — state indices change slowly

// RSMeans-based static material cost multipliers by state (vs. national avg = 1.00).
// Source: RSMeans 2023 City Cost Index — state-level approximations.
const STATIC_MATERIAL_MULTIPLIERS: Record<string, number> = {
  AK: 1.28, AL: 0.88, AR: 0.85, AZ: 0.95, CA: 1.22,
  CO: 1.02, CT: 1.14, DC: 1.18, DE: 1.08, FL: 0.96,
  GA: 0.92, HI: 1.35, IA: 0.96, ID: 0.90, IL: 1.12,
  IN: 0.98, KS: 0.92, KY: 0.90, LA: 0.91, MA: 1.18,
  MD: 1.08, ME: 0.98, MI: 1.04, MN: 1.05, MO: 0.96,
  MS: 0.84, MT: 0.94, NC: 0.90, ND: 1.00, NE: 0.94,
  NH: 1.06, NJ: 1.25, NM: 0.92, NV: 1.02, NY: 1.32,
  OH: 1.02, OK: 0.88, OR: 1.10, PA: 1.08, RI: 1.12,
  SC: 0.88, SD: 0.90, TN: 0.90, TX: 0.93, UT: 0.96,
  VA: 1.02, VT: 1.00, WA: 1.16, WI: 1.04, WV: 0.90,
  WY: 0.98,
};

// ZIP code prefix → state code lookup (first 3 digits of ZIP)
// Covers the major prefix ranges per USPS assignment.
const ZIP_PREFIX_TO_STATE: Record<string, string> = {
  "005": "MA", "010": "MA", "011": "MA", "012": "MA", "013": "MA", "014": "MA", "015": "MA", "016": "MA", "017": "MA", "018": "MA", "019": "MA",
...
export class LocationCostService {
  ...
  async getMultipliers(zipCode: string): Promise<LocationMultipliers> {
    const stateCode = this.zipToState(zipCode);
    if (!stateCode) {
      return this.buildMultipliers(zipCode, "US", 1.0, 1.0, "static");
    }

    // Try DB cache first
    try {
      const cached = await this.prisma.locationCostIndex.findUnique({
        where: { stateCode },
      });

      if (cached && cached.validUntil > new Date()) {
        return this.buildMultipliers(
          zipCode,
          stateCode,
          Number(cached.materialMultiplier),
          Number(cached.laborMultiplier),
          cached.dataSource.startsWith("oews") ? "oews" : "static",
        );
      }
    } catch {
      // DB read failed — continue to live fetch
    }

    // Fetch from BLS OEWS
    const results = await this.oews.fetchStateMultipliers(stateCode);
    const laborMult = this.oews.aggregateMultiplier(results);
    const materialMult = STATIC_MATERIAL_MULTIPLIERS[stateCode] ?? 1.0;
    const source = results[0]?.multiplier !== (STATIC_LABOR_MULTIPLIERS[stateCode] ?? 1.0) ? "oews" : "static";

    // Upsert in DB
    try {
      const validUntil = new Date(Date.now() + CACHE_TTL_MS);
      await this.prisma.locationCostIndex.upsert({
        where: { stateCode },
        update: { laborMultiplier: laborMult, materialMultiplier: materialMult, dataSource: `${source}+static`, validUntil, fetchedAt: new Date() },
        create: {
          stateCode,
      ...
      this.logger.warn(`Failed to cache location index for ${stateCode}: ${(err as Error).message}`);
    }

    return this.buildMultipliers(zipCode, stateCode, materialMult, laborMult, source as "oews" | "static");
  }

  /** Derive state code from ZIP prefix. Returns undefined if unknown. */
  zipToState(zipCode: string): string | undefined {
    const zip = zipCode.trim().replace(/\D/g, "").padStart(5, "0");
    return ZIP_PREFIX_TO_STATE[zip.slice(0, 3)];
  }

  private buildMultipliers(
    zipCode: string,
    stateCode: string,
...
export { STATIC_MATERIAL_MULTIPLIERS };
