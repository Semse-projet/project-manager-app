...
- `semse-agents-governance` — `packages/agents` ("Sistema de agentes") is unrelated to `ai-models/`'s Prometeo orchestrator; it contains 3-4 separate allow/deny/require_approval policy engines with no common interface (a risk the repo's own SPEC-GTW-F2 spec already flags, uncounted 4th engine included), and most of its "specialized agent" runtime builders are dead in production — only unit tests exercise them, while `apps/worker` routes real traffic through separate DB-backed handlers instead.
